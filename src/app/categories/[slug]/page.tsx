...
export default async function CategoryPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main><SiteHeader/><section className="page-hero"><div className="container"><span className="label">Category</span><h1>{category}</h1><p>{categoryDescription}</p></div></section><section className="section" aria-labelledby={`${slug}-tools-title`}><div className="container"><h2 id={`${slug}-tools-title`}>{category} tools for focused work</h2><p>{items.length} browser-based utilities are available in this category. Choose a tool below to inspect its supported workflow and run the transformation locally.</p><div className="tools-grid">{items.map(t=><ToolCard key={t.slug} tool={t}/>)}</div></div></section></main>
}
