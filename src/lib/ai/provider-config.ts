...
export function readAiConfig(): AiProviderConfig | undefined {
  ...
  try {
    ...
    if (
      result.value.providerId === "gemini" &&
      (result.value.model === "gemini-flash-latest" ||
        result.value.model === "gemini-2.5-flash" ||
        result.value.model === "gemini-2.0-flash")
    ) {
      return { ...result.value, model: "gemini-2.5-flash-lite" };
    }
    return result.value;
  } catch {
    return undefined;
