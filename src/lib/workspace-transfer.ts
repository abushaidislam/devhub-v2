...
export function buildWorkspaceExport(favorites:readonly string[],knownSlugs:ReadonlySet<string>):WorkspaceExport{const seen=new Set<string>();const clean:string[]=[];for(const slug of favorites){if(knownSlugs.has(slug)&&!seen.has(slug)){seen.add(slug);clean.push(slug)}}return {format:WORKSPACE_FORMAT,version:WORKSPACE_VERSION,exportedAt:new Date().toISOString(),containsUserInputs:false,favorites:clean}}
...
export function parseWorkspaceImport(raw:string,knownSlugs:ReadonlySet<string>):WorkspaceImportResult{
	...
	const seen=new Set<string>();const favorites:string[]=[];let skipped=0;
	for(const item of record.favorites){if(typeof item!=="string"||!knownSlugs.has(item)){skipped+=1;continue}if(seen.has(item))continue;seen.add(item);favorites.push(item)}
