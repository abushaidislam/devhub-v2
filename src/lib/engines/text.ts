...
export function convertCase(input:string):EngineResult{const value=ensureBatchInput(input,"Enter some text to convert.").trim();if(!value)throw new Error("Enter some text to convert.");const words=value.replace(/[_-]+/g," ").replace(/([a-z0-9])([A-Z])/g,"$1 $2").split(/\s+/).filter(Boolean).map(word=>word.toLowerCase());const camel=words.map((word,index)=>index===0?word:word[0].toUpperCase()+word.slice(1)).join("");const pascal=words.map(word=>word[0].toUpperCase()+word.slice(1)).join("");const lines=[`lower: ${value.toLowerCase()}`,`UPPER: ${value.toUpperCase()}`,`Title Case: ${words.map(word=>word[0].toUpperCase()+word.slice(1)).join(" ")}`,`camelCase: ${camel}`,`PascalCase: ${pascal}`,`snake_case: ${words.join("_")}`,`kebab-case: ${words.join("-")}`,`CONSTANT_CASE: ${words.join("_").toUpperCase()}`];return {output:lines.join("\n"),meta:`${words.length} words`}}
export function slugify(input:string):EngineResult{const value=ensureBatchInput(input,"Enter text to turn into a slug.").trim();if(!value)throw new Error("Enter text to turn into a slug.");const slug=value.normalize("NFKD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"");if(!slug)throw new Error("This text contains no characters usable in a URL slug.");return {output:slug,meta:`${slug.length} characters`}}
export function diffLines(input:string):EngineResult{const value=ensureBatchInput(input,"Separate the two versions with a line containing only ---.");const separator=/^\s*---\s*$/m;if(!separator.test(value))throw new Error("Separate the two versions with a line containing only ---.");const [left="",right=""]=value.split(separator);const a=left.replace(/^\n+|\n+$/g,"").split("\n");const b=right.replace(/^\n+|\n+$/g,"").split("\n");const lines:string[]=[];let added=0;let removed=0;const max=Math.max(a.length,b.length);for(let index=0;index<max;index+=1){const before=a[index];const after=b[index];if(before===after){lines.push(`  ${before??""}`);continue}if(before!==undefined){lines.push(`- ${before}`);removed+=1}if(after!==undefined){lines.push(`+ ${after}`);added+=1}}return {output:lines.join("\n"),meta:`${added} added, ${removed} removed`}}
export function analyzeText(input:string):EngineResult{const value=ensureBatchInput(input,"Enter text to analyze.");const words=value.trim()?value.trim().split(/\s+/).length:0;const lines=value?value.split("\n").length:0;const sentences=value.split(/[.!?]+/).filter(s=>s.trim().length>0).length;const characters=[...value].length;const readingMinutes=Math.max(1,Math.round(words/200));const output=[`Characters: ${characters}`,`Characters (no spaces): ${[...value.replace(/\s/g,"")].length}`,`Words: ${words}`,`Lines: ${lines}`,`Sentences: ${sentences}`,`Reading time: ~${readingMinutes} min at 200 wpm`].join("\n");return {output,meta:`${words} words`}}
